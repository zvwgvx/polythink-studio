...
function App() {
  ...
  const handleSelectDataset = async (dataset) => {
    ...
    if (editingItemIndex !== null) {
      // For simplicity in dataset switching, we just alert. 
      // Implementing the modal here would require more complex state management to resume the switch action.
    ...
    try {
      ...
      const data = await api.getDatasetContent(dataset.type, filename);
  ...
  const startEditingItem = (globalIdx) => {
    if (editingItemIndex !== null && editingItemIndex !== globalIdx) {
      // We can use the modal here too if we want, but window.confirm is simpler for this edge case
        ...
        <div className="space-y-6 pb-20">
          ...
          <div className="flex justify-between items-center sticky top-0 bg-[#121212]/95 backdrop-blur z-20 py-6 border-b border-[#333]">
            ...
            <div className="space-x-4 flex items-center">
              {viewMode === 'json_edit' ? (
                <>
                  <Button variant="secondary" size="sm" onClick={handleExitJsonMode}>Cancel</Button>
                  <Button
                    variant="success"
                    size="sm"
